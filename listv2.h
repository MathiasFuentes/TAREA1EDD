...
// Función booleana que verifica si un cierto ID se encuentra en la lista.
...
// Mostrar los tickets en orden de prioridad, desde la más alta hasta la más baja

void procesarTicket(list *list);
// Muestra los datos del primer ticket en la lista y los muestra.

void searchTicket(list *list, size_t id);
// Busca un ticket y muestra los datos del mismo.

/*
------------------------------------------------------------------------
FUNCIONES DE USO EXCLUSIVO DEL MAIN QUE EJECUTAN LAS FUNCIONES ANTERIORES
-------------------------------------------------------------------------
*/

void insertarTicket(list *list);
void asignarPrioridadTicket(list *list);
void buscarTicket(list *list);
void liberarMemoria(list *list);
